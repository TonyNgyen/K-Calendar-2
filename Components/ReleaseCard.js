...
export default function ReleaseCard(props) {
  ...
  return (
    <div className="rounded-2xl mx-auto overflow-hidden max-w-sm bg-white shadow-lg shadow-gray-300 font-poppins pb-5">
      ...
      <div className="flex items-center pl-5 pt-3">
        <Link className='group text-black transition-all duration-300 ease-in-out mr-3' href={{
            ...
            <span className='text-3xl bg-left-bottom bg-gradient-to-r from-lightmodeRed to-lightmodeRed bg-[length:0%_2px] bg-no-repeat group-hover:bg-[length:100%_2px] transition-all duration-500 ease-out text-lightmodeRed'>
              {props.artist}
            </span>
        </Link>
        <h4 className="bg-lightmodeRed px-2 inline rounded-lg text-white text-md">{day + (day>10 && day<20 ? 'th' : {1:'st', 2:'nd', 3:'rd'}[day % 10] || 'th')}</h4>
      </div>
