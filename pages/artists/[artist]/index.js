    ...
    <>
      ...
      <div className="grid lg:grid-cols-6 md:grid-cols-4 justify-items-center grid-cols-2 gap-5 lg:px-20 md:px-15 px-10 mb-20">
        {groupData.releases.albums.map((album) => (
          <ReleaseDisplay image={album["release image"]} name={album["release name"]} artistName={groupData.name} type={"albums"} index={groupData.releases.albums.indexOf(album)}></ReleaseDisplay>
        ))}
      </div>
      ...
      <div className="grid lg:grid-cols-6 md:grid-cols-4 justify-items-center grid-cols-2 gap-5 lg:px-20 md:px-15 px-10 mb-20">
        {groupData.releases.eps.map((ep) => (
          <ReleaseDisplay image={ep["release image"]} name={ep["release name"]} artistName={groupData.name} type={"eps"} index={groupData.releases.eps.indexOf(ep)}></ReleaseDisplay>
        ))}
      </div>
      ...
      <div className="grid lg:grid-cols-6 md:grid-cols-4 justify-items-center grid-cols-2 gap-5 lg:px-20 md:px-15 px-10 mb-20">
        {groupData.releases.singles.map((single) => (
          <ReleaseDisplay image={single["release image"]} name={single["release name"]} artistName={groupData.name} type={"singles"} index={groupData.releases.singles.indexOf(single)}></ReleaseDisplay>
        ))}
      </div>
      ...
      <div className="grid lg:grid-cols-6 md:grid-cols-4 justify-items-center grid-cols-2 gap-5 lg:px-20 md:px-15 px-10 mb-20">
        {groupData["related artists"].map((artist) => (
          <ArtistDisplay image={artist.image} name={artist.name}></ArtistDisplay>
        ))}
      </div>
    </>
  )
